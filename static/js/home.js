...
function create_calendar(date) {
   for (let mm = 0; mm < 3; mm++) {
      ...
      mounth_li.innerText = `${Number(date.getMonth()) + (mm)}月`
...
function add_calendar(date, front_flg) {
   ...
   mounth_li.innerText = `${Number(date.getMonth() +1) }月`
   mounth_li.className = 'sticky-li'
...
function set_schedule(start_date, end_date) {
   ...
   fetch((fetch_url), {
      ...
      data.items.forEach(element => {
         ...
         // let mstartTime = timeFormat(eventDate)
         // let firstchild = document.getElementById('cal').firstElementChild ;
         // console.log(`${('00' + (eventDate.getMonth() )).slice(-2)}`)
         // console.log(`${Number(firstchild.id)}`)
         // while (`${eventDate.getFullYear()}${('00' + (eventDate.getMonth()-1)).slice(-2)}99` < firstchild.id){
         ...
         //    add_calendar(new Date(lastchild.id.substr( 0, 4 ),Number(lastchild.id.substr( 4, 2 )) +1  ),false)
         //    lastchild = document.getElementById('cal').lastElementChild
         // }

         // console.log(formatDate(eventDate))
         // console.log(document.getElementById(formatDate(eventDate)))
         // const addevent = document.getElementById(formatDate(eventDate))
         // addevent.style.paddingBottom = '1em'
         // let timeCount = end_Date.getTime() - eventDate.getTime()
         ...
         //          date.innerHTML = date.innerHTML + `<div class="schedule-text"><span style="color:#${element['color']};margin-right: 0.5em">●</span>${mstartTime}〜24:00 ${element['title']}</div>`
         //          timeCount = timeCount - 86400000
         //          mstartTime = '00:00'
         //       }

         //       document.getElementById(formatDate(end_Date)).innerHTML = document.getElementById(formatDate(end_Date)).innerHTML + `<div class="schedule-text"><span style="color:#${element['color']};margin-right: 0.5em">●</span>${mstartTime}〜${timeFormat(end_Date)}  ${element['title']}</div>`
         //    } else {
         //       addevent.innerHTML = addevent.innerHTML + `<div class="schedule-text"><span style="color:#${element['color']};margin-right: 0.5em">●</span>${timeFormat(eventDate)}〜${timeFormat(end_Date)}  ${element['title']}</div>`
...
function local_set_schedule(start_date, end_date,title,color) {
